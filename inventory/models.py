...
class Receive(models.Model):
    date = models.DateTimeField()
    number = models.CharField(max_length=20)
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="receive")
    # warehouse = models.ForeignKey(warehouse,on_delete=models.CASCADE, related_name="receive")
    user = models.ForeignKey(User, on_delete=models.CASCADE, related_name="receive")

    def __str__(self):
        return f"Receive {self.number} ({self.date})"

class ReceiveLine(models.Model):
    receive = models.ForeignKey(Receive, on_delete=models.CASCADE, related_name="products")
    product = models.ForeignKey(Product, on_delete=models.CASCADE, related_name="receive_lines")
    qty = models.DecimalField(max_digits=10, decimal_places=3)
    price = models.DecimalField(max_digits=15, decimal_places=2)
    amount = models.DecimalField(max_digits=15, decimal_places=2)
